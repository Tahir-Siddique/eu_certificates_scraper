...
df = pd.read_csv("certificates6.csv")

data = df.to_dict('records')
_data = pd.read_csv("~/eu_certificates_scraper/output6.csv").to_dict("records")
    ...
    for idx, item in enumerate(data):
        try:
            ...
            if first_link_in_table:
                ...
                _data.append(d)    
                pd.DataFrame(_data).to_csv("~/eu_certificates_scraper/output6.csv", index=False)
            open_the_turnstile_page(sb)
